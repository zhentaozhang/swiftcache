package io.swiftcache.core.map;
...
public class MyProgressiveReHashMap<K,V> extends AbstractMap<K,V> implements Map<K,V> {
    ...
    private static final Logger log = LoggerFactory.getLogger(MyProgressiveReHashMap.class);
    ...
    public V put(K key, V value) {
        ...
        if(!isInRehash) {
            //1. 是否为更新
            AbstractMap.SimpleEntry<Boolean, V> pair = updateTableInfo(key, value, this.table, this.capacity);
            if(pair.getKey()) {
            ...
            AbstractMap.SimpleEntry<Boolean, V> pair2 = updateTableInfo(key, value, this.rehashTable, this.rehashCapacity);
            if(pair2.getKey()) {
                V oldVal = pair2.getValue();
     ...
     * @since 0.0.3
     */
    private AbstractMap.SimpleEntry<Boolean, V> updateTableInfo(K key, V value, final List<List<Entry<K,V>>> table,
